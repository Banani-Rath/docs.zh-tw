...
您可以藉由儲存屬性值或擷取其值來呼叫屬性程序，  您可以利用存取變數的方式來存取屬性。  
 ...
 屬性的 `Set` 程序可儲存值，且其 `Get` 程序可擷取值。  然而，您不會依名稱明確地呼叫這些程序。  您可以使用指派陳述式或運算式來使用屬性，如同儲存或擷取變數值一般。  [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] 會呼叫屬性的程序。  
