...
為了可以當成最少運算 \(Short Circuit\) 運算子使用，使用者定義的邏輯運算子 \('operator'\) 其傳回類型必須與其 2 個參數的類型相同。  
 ...
 如果您定義了一個使用者定義類型的運算子，然後嘗試使用該運算子作為最少運算運算子，則使用者定義運算子必須具有相同類型的參數和傳回值。 如需最少運算運算子的詳細資訊，請參閱 [&& 運算子](../../csharp/language-reference/operators/conditional-and-operator.md)和 [&#124;&#124; 運算子](../../csharp/language-reference/operators/conditional-or-operator.md)。  
